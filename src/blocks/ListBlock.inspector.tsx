...
export default function ListBlockInspector({ blockId, editor }: { blockId: string; editor: any }) {
  const [runtimeState, setRuntimeState] = useAtom(blockRuntimeAtom(blockId))
  const triggerSave = useSetAtom(triggerSaveAtom)
  const store = useStore()

  // List only DatabaseBlocks on the canvas
  const canvasBlocks = useMemo(() => {
    return getCanvasBlocks(editor, store, blockId).filter(b => b.type === 'databaseBlock')
    ...
    <div>
      <label style={{ display: 'block', marginBottom: '12px' }}>
        ...
        <input
          ...
          placeholder={getBlockTypeDisplayName('listBlock')}
