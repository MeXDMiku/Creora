...
import { blockRuntimeAtom, blockPositionAtom, activeWireAtom, snapTargetAtom, workflowsAtom, triggerSaveAtom, selectedBlockIdAtom, connectionsAtom, contextMenuAtom, getPortBadge } from '../state/atoms';
import { useMemo, useRef, useState } from 'react';
import { useBlockDrag } from '../hooks/useBlockDrag';

const ButtonBlockComponent = (props: NodeViewProps) => {
  const { node } = props;
  const { label, blockId } = node.attrs;
...
export const ButtonBlock = Node.create({
  ...
  addAttributes() {
    return {
      blockId: {
        default: () => 'btn_' + Math.random().toString(36).substring(2, 11),
      },
      label: {
        default: 'Click me',
      },
  ...
  renderHTML({ HTMLAttributes }) {
    return ['div', { 'data-type': 'button-block', ...HTMLAttributes }];
  },

  addNodeView() {
    return ReactNodeViewRenderer(ButtonBlockComponent, { as: 'div' });
  },
});
